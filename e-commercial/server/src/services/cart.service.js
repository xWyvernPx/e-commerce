...
class CartService {
    async getCartByAccount(accountId) {
     ...
     .then(data => {
         if (data)
         return CartItem.findAll({where : {sessionID : data.id},include : [Product]}).then(data=> data.map(ele => {
             const product = ele.Product;
             const {name,instock,describe,brand,from_province,from_district,SKU,price,rating} = product;
                return {
             id : ele.id,
             quantity : ele.quantity,
            product : {
                productID : product.id,
                name,instock,describe,brand,from_province,from_district,SKU,price,rating
            }
         }
        }))
