...
function InlineCart(props) {
  ...
  const cartList = useSelector((state) => state.rootReducer.cartReducer.cart);
  useEffect(() => {
    console.log("oke reset cart List in InlineCart");
    if (cartList) amountSet(cartList.length);
  }, [cartList, amountSet]);
  ...
  return (
    <div className={`checkout__cart ${active ? "cartActive" : ""}`}>
      <div className="checkout__header">
        ...
        <img
          src="https://ik.imagekit.io/flamefoxeswyvernp/Project/UI_challenge/e-commecial/ic-actions-close-simple_R1SCIuwbt.svg?updatedAt=1639053398420"
          alt=""
      ...
      <div className="checkout__list">
        {cartList &&
          cartList.map((item) => <CartProduct key={item.id} item={item} />)}
      ...
      <div className="checkout__selection">
        <a href="#!" className="checkout__continue">
          Continue shopping
        </a>
        <a href="#!" className="checkout__checkout">
          Go to checkout
        </a>
      </div>
    </div>
